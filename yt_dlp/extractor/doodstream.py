...
class DoodStreamIE(InfoExtractor):
    IE_NAME = 'DoodStream'
    _VALID_URL = r'https?://(?:www\.)?dood(s)?\.(?:to|com|watch|so|pm|wf|yt|pro)/(?P<type>[ed])/(?P<id>[a-z0-9]+)'
    _TESTS = [{
...
class Ds2playIE(InfoExtractor):
    ...
    _VALID_URL = r'https?://(?:www\.)?ds2play\.(?:com|to|watch|so|pm|wf|yt|pro)/(?P<type>[ed])/(?P<id>[a-z0-9]+)'
    _TESTS = [{
